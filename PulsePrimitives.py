...
def Utheta(qubit, amp=0, phase=0, **kwargs):
    ...
    params = overrideDefaults(qubit, kwargs)
    shape = params['shapeFun'](amp=amp, phase=phase, **params)
    return Pulse("Utheta", (qubit), shape, 0.0)

#Setup the default 90/180 rotations
# @_memoize
def X(qubit):
    shape = qubit.shapeFun(amp=qubit.piAmp, phase=0, **overrideDefaults(qubit, {}))
    return Pulse("X", (qubit), shape, 0.0)
    
# @_memoize
def X90(qubit):
    shape = qubit.shapeFun(amp=qubit.pi2Amp, phase=0, **overrideDefaults(qubit, {}))
    return Pulse("X90", (qubit), shape, 0.0)

# @_memoize
def Xm(qubit):
    shape = qubit.shapeFun(amp=qubit.piAmp, phase=0.5, **overrideDefaults(qubit, {}))
    return Pulse("Xm", (qubit), shape, 0.0)
    
# @_memoize
def X90m(qubit):
    shape = qubit.shapeFun(amp=qubit.pi2Amp, phase=0.5, **overrideDefaults(qubit, {}))
    return Pulse("X90m", (qubit), shape, 0.0)

# @_memoize
def Y(qubit):
    shape = qubit.shapeFun(amp=qubit.piAmp, phase=0.25, **overrideDefaults(qubit, {}))
    return Pulse("Y", (qubit), shape, 0.0)

# @_memoize
def Y90(qubit):
    shape = qubit.shapeFun(amp=qubit.pi2Amp, phase=0.25, **overrideDefaults(qubit, {}))
    return Pulse("Y90", (qubit), shape, 0.0)

# @_memoize
def Ym(qubit):
    shape = qubit.shapeFun(amp=qubit.piAmp, phase=0.75, **overrideDefaults(qubit, {}))
    return Pulse("Ym", (qubit), shape, 0.0)

# @_memoize
def Y90m(qubit):
    shape = qubit.shapeFun(amp=qubit.pi2Amp, phase=0.75, **overrideDefaults(qubit, {}))
...
def CNOT(source, target):
    ...
    shape = source.shapeFun(amp=source.piAmp, phase=0.0, **overrideDefaults(source, {}))
