...
class VRAMentry(object):
	...
	def test_valid(self, mask):
		"""Test validity using mask: bits which are 1 in both mask and stored
		validity must match"""
...
class APS2Sequencer:
	...
	Currently there is no support for SYNC and WAIT instructions as this
	simulation ignores timing.

	TODO:
	- Can we support SYNC/WAIT better?
	- Can we support the TDM message queue? Eventually I would like to move
		this to an async execution model to better simulate multiple APS
		units...
	- Support modulation commands.
	- Double check spec for comparison implementation (not sure there is any
		functional difference from here...)
	"""
