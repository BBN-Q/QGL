...
def delay(linkList, delay, samplingRate):
    ...
    sampShift = round(delay * samplingRate)
...
def create_gate_seqs(linkList, gateBuffer=0, gateMinWidth=0, samplingRate=1.2e9):
    ...
    gateMinWidth = round(gateMinWidth * samplingRate)
    ...
    for miniLL in linkList:
        ...
        for ct in range(len(miniLL)-1):
            entryWidth = miniLL[ct].length * miniLL[ct].repeat - carry
            assert entryWidth>0
            ...
            if state == 0 and not miniLL[ct+1].isZero:
                ...
                state = 1
            elif state == 1 and miniLL[ct+1].isZero and (miniLL[ct+1].length*miniLL[ct+1].repeat) > gateMinWidth:
